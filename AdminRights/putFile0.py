...
def deploy_file_to_host(token, batch_id, sha256, remote_file_path, deploy_url):
    headers = {
        'Authorization': f'Bearer {token}',
        'Content-Type': 'application/json'
    }

    # Adjust the command_string if necessary
    command_string = f"put {os.path.basename(remote_file_path)} {remote_file_path}"
    # Alternate (if simpler path works)
    ...
    if response.status_code in range(200, 300):
        ...
        print("Failed to deploy file to host: ", response.text)
